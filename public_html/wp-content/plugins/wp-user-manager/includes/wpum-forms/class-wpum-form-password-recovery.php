...
class WPUM_Form_Password_Recovery extends WPUM_Form {
		...
		$this->fields = apply_filters( 'password_recover_form_fields', array(
			'user' => array(
				'username_email' => array(
					'label'       => __( 'Email', 'wp-user-manager' ),
					'type'        => 'text',
					'required'    => true,
					'placeholder' => '',
					'priority'    => 1
	...
	public function submit( $atts ) {
		...
		$data = [
			...
			'step'    => $this->get_step(),
			'message' => apply_filters( 'wpum_lost_password_message', esc_html__( 'Enter the email you provided when creating. We will send a letter to him with further instructions.', 'wp-user-manager' ) )
		];
